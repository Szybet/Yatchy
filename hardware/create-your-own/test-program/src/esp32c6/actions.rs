...
use crate::esp32c6::flex_io::FlexIo;
...
pub async fn reset_actions(io: &mut FlexIo<'_>) {
    crate::esp32c6::gpio_action::gpio_reset(io).await;
}
    ...
    if let Some(action) = act {
        match action {
            CurrentAction::Gpio(pin) => {
                crate::esp32c6::gpio_action::gpio_action(*pin, io).await;
            }
            CurrentAction::SelfCheckGpio() => {
                crate::esp32c6::gpio_action::self_check_gpio(act, io).await;
            }
        }
    }
}
